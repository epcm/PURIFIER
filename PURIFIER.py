# 这是一个用于发布exe的版本
# https://gist.github.com/AnthonyBriggs/cac72989c2dd3c4aeb7475237079d2fb
# 主要区别在csv写入，wav格式音频，字体设置
# 需要在目录加入pgzero和easygui文件夹
import pgzrun
import random
import time
import math
...
pygame.mixer.music.load("dalabengba.wav")
...
class Box(Actor):
    ...
    def open_box(self):
        ...
        self.open = True
        if self.type == 1:
            mon = Monster('orange_dino','5','220','1','0.05','1.5','fireball','1.2','200')
            mon.pos = self.pos
            monsters.append(mon)
        elif self.type == 2:
            mon = Monster('green_dino','5','220','1','0.05','1','leaf','1.5','200')
            mon.pos = self.pos
            monsters.append(mon)
        elif self.type == 3:
            mon = Monster('red_dino','5','220','1.5','0.05','1','fireball1','1.2','200')
            mon.pos = self.pos
            monsters.append(mon)
...
def construct_level():
    ...
    if level == 1:
        ...
        mon = Minotaur('minotaur','10','220','2.5','0.1','1','axe','1.2','200')
        mon.land_center = x, y
...
MP_RECOVERY_SPEED = 0.1 #每次刷新的MP恢复量
...
class Boss(Actor):
    def __init__(self):
        ...
        self.pos1 = (WIDTH - 395, 265)#395
        self.pos2 = (WIDTH - 401, 389) #401
        self.HP = HP(200, 1)
        self.timer = 0
...
fuzi = Weapon('斧子','1','60','2','0','10','-1','-1','Normal')
gong1 = Weapon('弓1','2','200','1.5','0','10','箭','1.5','Normal')
gong2 = Weapon('弓2_trans','2','200','1.5','30','15','箭_ice','1.5','Slow Down the Monster') # 半透明代表未拥有
jian1= Weapon('剑1_trans','1','75','5','30','15','-1','-1','High Damage')
jian2 = Weapon('剑2_trans','1','75','3','30','15','-1','-1','Long Beaten Distance')
qiang1 = Weapon('枪1_trans','2','200','1','40','20','子弹特效1','1.5','Multiple Bullets')
qiang2 = Weapon('枪2_trans','2','200','3','40','20','子弹特效2','2.5','Improved Attack')
changmao1 = Weapon('长矛1_trans','2','500','3','50','30','长矛1_bullet','1.5','Trace Your Nearest Enemy')
changmao2 = Weapon('长矛2_trans','2','300','0.2','200','40','长矛2_bullet','2','Penetrating Attack')
fuzi.pos = (370,240)
gong1.pos = (405,240)
gong2.pos = (445,240)
jian1.pos = (480,240)
...
def draw_coins_bar():
    ...
    screen.draw.text(str(coins), (125, 97), fontsize=50,fontname = '09',color = 'gold')  
...
def draw():
    ...
    if game:
        ...
        if step == 2:
            screen.clear()
            screen.fill('white')
            screen.draw.text("You have lost your game, please exit!", (300, 300), fontname = '09',fontsize=50, color="orange")
            '''FM.close()
            FW.close()
            FG.close()'''
        ...
        screen.draw.text("Press Number 2 to exit the game", (800, 20), fontsize=25, fontname = '09',color='orange')
        screen.draw.text(f"Lives: {hero_HP.num - hero_HP.count + 1}", (375, 20), fontsize=40, fontname = '09',color='red')
    ...
    if step_store in range(2,11) and not bag_open:
        ...
        screen.draw.text("Price: %d "%weapons[step_store-2].price,(x1,y1),fontsize=50,fontname = '09',color = "gold")
        screen.draw.text(weapons[step_store-2].Note,(x0, y0),fontsize=50,color = "gold",fontname = '09')

    if step_store1 == 20:
        screen.draw.text("Your coins are not ENOUGH!",(300,400),fontsize=50,color = "red",fontname = '09')
